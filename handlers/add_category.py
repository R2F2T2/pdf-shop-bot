...
async def start_add_category(callback: types.CallbackQuery, state: FSMContext):
    ...
    if data == None:
        return
    instr_msg = await callback.message.answer("Введите название категории:")
    await state.update_data(
        cat_msg_id=callback.message.message_id,
        instr_msg_id=instr_msg.message_id
        )

    logger.info(f"Запущена процедура создание новоей категории!")    
...
async def add_category_name(message: types.Message, state: FSMContext):
    data = await state.get_data()
    db_result = await db.add_category(message.text)        
